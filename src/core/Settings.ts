...
class Settings {
    ...
    /**
     * Add a plugin to a specific group section
     */
    public static addPluginToGroup(fileName: string, metaData: MetaData, groupId: string): void {
        this.logger.info(`Adding plugin: ${fileName} to group: ${groupId}`);
        ...
        if (groupContent) {
            groupContent.appendChild(pluginContainer);
        } else {
            this.logger.warn(`Plugin group content not found: ${groupId}-content`);
            // Fallback to regular plugins category
            const pluginsCategory = document.querySelector(SELECTORS.PLUGINS_CATEGORY);
            pluginsCategory?.appendChild(pluginContainer);
        }
