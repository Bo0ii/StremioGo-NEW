...
import { writeFileSync, readdirSync, statSync, existsSync, mkdirSync, unlinkSync } from "fs";
...
class ModManager {
     ...
     * Uses async file reading to avoid blocking the renderer
     */
    public static async loadPlugin(pluginName: string): Promise<void> {
        ...
        const userPluginPath = join(properties.pluginsPath, pluginName);
        const bundledPluginPath = join(Properties.bundledPluginsPath, pluginName);
        const pluginPath = existsSync(userPluginPath) ? userPluginPath : bundledPluginPath;

        if (!existsSync(pluginPath)) {
            this.logger.error(`Plugin file not found in user or bundled paths: ${pluginName}`);
            return;
        }

        try {
            // Use async file reading to avoid blocking the renderer
            const plugin = await readFile(pluginPath, "utf-8");
            ...
            this.logger.info(`Plugin ${pluginName} loaded from ${pluginPath}`);
        } catch (error) {
    ...
    public static unloadPlugin(pluginName: string): void {
        // Prevent unloading always-enabled plugins
        const ALWAYS_ENABLED_PLUGIN = "card-hover-info.plugin.js";
        if (pluginName === ALWAYS_ENABLED_PLUGIN) {
            this.logger.info(`Plugin ${pluginName} cannot be disabled (always enabled)`);
            return;
        }
    ...
    private static getInstalledThemes(): string[] {
        // Sync version for compatibility - used by isThemeInstalled which is called synchronously
        const userThemes = existsSync(Properties.themesPath)
            ? readdirSync(Properties.themesPath).filter(file => statSync(join(Properties.themesPath, file)).isFile())
            : [];
        const bundledThemes = existsSync(Properties.bundledThemesPath)
            ? readdirSync(Properties.bundledThemesPath).filter(file => statSync(join(Properties.bundledThemesPath, file)).isFile())
            : [];
        // Merge without duplicates
        return [...new Set([...userThemes, ...bundledThemes])];
    }

    private static getInstalledPlugins(): string[] {
        // Sync version for compatibility - used by isPluginInstalled which is called synchronously
        const userPlugins = existsSync(Properties.pluginsPath)
            ? readdirSync(Properties.pluginsPath).filter(file => statSync(join(Properties.pluginsPath, file)).isFile())
            : [];
        const bundledPlugins = existsSync(Properties.bundledPluginsPath)
            ? readdirSync(Properties.bundledPluginsPath).filter(file => statSync(join(Properties.bundledPluginsPath, file)).isFile())
            : [];
        // Merge without duplicates
        return [...new Set([...userPlugins, ...bundledPlugins])];
    }
    ...
    public static togglePluginListener(): void {
        helpers.waitForElm(SELECTORS.PLUGINS_CATEGORY).then(() => {
            ...
            // Plugin that should always be enabled
            const ALWAYS_ENABLED_PLUGIN = "card-hover-info.plugin.js";
            
            for (let i = 0; i < pluginCheckboxes.length; i++) {
                const pluginName = pluginCheckboxes[i].getAttribute('name');
                
                // Make card-hover-info always enabled and disable the checkbox
                if (pluginName === ALWAYS_ENABLED_PLUGIN) {
                    pluginCheckboxes[i].classList.add(CLASSES.CHECKED);
                    (pluginCheckboxes[i] as HTMLElement).style.opacity = "0.6";
                    (pluginCheckboxes[i] as HTMLElement).style.cursor = "not-allowed";
                    (pluginCheckboxes[i] as HTMLElement).title = "This plugin is always enabled";
                    continue; // Skip adding event listener for this plugin
                }
                
                pluginCheckboxes[i].addEventListener("click", () => {
                    pluginCheckboxes[i].classList.toggle(CLASSES.CHECKED);
                    const clickedPluginName = pluginCheckboxes[i].getAttribute('name');

                    if (!clickedPluginName) return;

                    if (pluginCheckboxes[i].classList.contains(CLASSES.CHECKED)) {
                        this.loadPlugin(clickedPluginName);
                    } else {
                        this.unloadPlugin(clickedPluginName);
    ...
    public static async checkForItemUpdates(itemFile: string): Promise<void> {
        ...
        const bundledPath = join(
            pluginOrTheme === "theme" ? Properties.bundledThemesPath : Properties.bundledPluginsPath,
            itemFile
        );
        const itemPath = existsSync(userPath) ? userPath : bundledPath;
